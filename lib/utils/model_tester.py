...
from lib.utils.reduce_to_min_important_features import to_important_features
...
class ModelTester:
    ...
    __slots__ = (
        'clfs', 'clf_args', 'data_cols', 'split_data', 'split_data_norm', 'handlers', 'classifiers_names_', 'results_'
    )
    ...
    def plot_importances(self, std_dev_mult=1, top=False):
        if top:
            min_top_imp = reduce(to_important_features(std_dev_mult=std_dev_mult), self.feature_importances.items(), {})
        ...
        for n, res in enumerate(features):
            imp = res.get('feature_ranks')[min_top_imp.keys()] if top else res.get('feature_ranks')
            if not imp.empty:
                ax = axs[n]
